...
Installer.prototype.install = function *() {
  var pkgs = yield this.dependencies(this.local, this._manifest);
...
Installer.prototype.dependencies = function *(json, parent, out) {
  ...
  // fetch the slug
  function slug(pkg) {
    return join(pkg.slug(), self._manifest);
  }
  ...
  function recurse(pkg, i) {
    ...
    var gens =  self.dependencies(json, join(pkg.slug(), self._manifest), out);
    return gens;
  }
